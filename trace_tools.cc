...
int header_gen(int d, FilterList* filters, FILE *fp, float a, float b, int scale){
  ...
  while(num_headers < threshold){
    ...
    int RandFilt = 0;
    if (p < 0.4)
        RandFilt = randint((int)((fsize - 1) * 0.6), (int)((fsize - 1) * 0.7));
        //RandFilt = randint(0, (int)((fsize - 1) * 0.1));
    else if (p > 0.6)
        RandFilt = randint((int)((fsize - 1) * 0.1), (int)((fsize - 1) * 0.2));
        //RandFilt = randint(0, (int)((fsize - 1) * 0.1));
        //RandFilt = randint((int)((fsize - 1) * 0.9), fsize-1);
    else
        RandFilt = randint(0, fsize - 1);
        //RandFilt = randint((int)((fsize - 1) * 0.1), (int)((fsize - 1) * 0.9));
...
//      p = drand48();
//      new_hdr[i] = (unsigned)((filts->high(RandFilt, i) -
//                               filts->low(RandFilt, i)) *
//                               p + filts->low(RandFilt, i));
  }
  return;
}
